    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.expenses.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cs_expenses') ? 'is-invalid' : '' }}" name="cs_expenses_id" id="cs_expenses_id">
                    @foreach($cs_expenses as $id => $entry)
                        <option value="{{ $id }}" {{ old('cs_expenses_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('cs_expenses'))
                    <span class="text-danger">{{ $errors->first('cs_expenses') }}</span>
                @endif
