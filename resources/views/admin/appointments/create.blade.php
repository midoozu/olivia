...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.appointments.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="payType">{{ trans('cruds.appointment.fields.payType') }}</label>
                <select class="form-control select2 {{ $errors->has('payType') ? 'is-invalid' : '' }}" name="payType" id="payType">
                    @foreach($payType as $id => $entry)
                        <option value="{{ $id }}" {{ old('payType') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('payType'))
                    <span class="text-danger">{{ $errors->first('payType') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.appointment.fields.payType_helper') }}</span>
            </div>
            ...
            @can('followup')
                <div class="form-group">
                    <div class="form-check {{ $errors->has('follow_up') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="follow_up" id="follow_up" value="1" {{ old('follow_up', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('check_in') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="check_in" id="check_in" value="1" {{ old('check_in', 0) == 1 ? 'checked' : '' }}>
