...
@can('att_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.atts.create') }}">
                ...
                <tbody>
                    @foreach($atts as $key => $att)
                        <tr data-entry-id="{{ $att->id }}">
                            ...
                            <td>
                                @can('att_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.atts.show', $att->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('att_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.atts.edit', $att->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('att_delete')
                                    <form action="{{ route('admin.atts.destroy', $att->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('att_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.atts.massDestroy') }}",
