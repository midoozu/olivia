...
<form action="{{route('hrPredecessor.update',$find->id)}}" method="post" id="Form">
...
<div class="row">
    ...
    <div class="col-6 mb-4">
        ...
        <input name="value"  value="{{$find->value}}" type="text" id="mainValue" placeholder=" المبلغ..."  class="form-control numbersOnly" data-validation="required" >
    </div>
    <div class="col-6 mb-4">
        <label class="label mb-2 "> تاريخ السلفة   </label>
        <input name="date"   type="date" class="form-control" value="{{$find->date}}"
    ...
    <input name="pay_status" type="hidden" value="new">
        ...
        <div class="col-12 mb-4" id="Batches" style="display:{{$find->type == 'regular'?'block':'none'}};">
            ...
            <div class="table-responsive-md col-sm-12">
                <table class="table table-striped-table-bordered table-hover table-checkable table-" id="tbl_posts">
                    ...
                    @foreach($find->details_rl as $key => $details)
                        <tr id="rec-{{$key + 1}}" class="MainDivs">
                            <td><span class="sn">{{$key + 1}}</span>.</td>
                            <td>
                                <input type="text"  name="value_[]" value="{{$details->value}}" data-validation="required" class="form-control  numbersOnly smallValue" data-validation="required"  placeholder=" المبلغ..." >
                            </td>
                            <td>
                                <input type="date"  name="date_[]" value="{{$details->date}}"  class="form-control"  data-validation="required" >
                            </td>
