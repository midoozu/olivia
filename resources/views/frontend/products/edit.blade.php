                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.products.update", [$product->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="size_id" id="size_id">
                                @foreach($sizes as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('size_id') ? old('size_id') : $product->size->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('size'))
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="qty" id="qty" value="{{ old('qty', $product->qty) }}" required>
                            @if($errors->has('qty'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('qty') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="atts[]" id="atts" multiple>
                                @foreach($atts as $id => $att)
                                    <option value="{{ $id }}" {{ (in_array($id, old('atts', [])) || $product->atts->contains($id)) ? 'selected' : '' }}>{{ $att }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('atts'))
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="inv_name_id" id="inv_name_id" required>
                                @foreach($inv_names as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('inv_name_id') ? old('inv_name_id') : $product->inv_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('inv_name'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('inv_name') }}
                                </div>
                            @endif
