                                    ...
                                    <tr data-entry-id="{{ $productTag->id }}">
                                        ...
                                        <td>
                                            @can('product_tag_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.product-tags.show', $productTag->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('product_tag_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.product-tags.edit', $productTag->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('product_tag_delete')
                                                <form action="{{ route('frontend.product-tags.destroy', $productTag->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
