                                    ...
                                    <tr data-entry-id="{{ $assetStatus->id }}">
                                        ...
                                        <td>
                                            @can('asset_status_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.asset-statuses.show', $assetStatus->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('asset_status_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.asset-statuses.edit', $assetStatus->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('asset_status_delete')
                                                <form action="{{ route('frontend.asset-statuses.destroy', $assetStatus->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
